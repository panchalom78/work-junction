...
const AdminPayments = () => {
  ...
  const getItemsPerPage = () => {
    if (screenSize.isMobile) return 6;
    if (screenSize.isTablet) return 9;
    return 12;
  };

  useEffect(() => {
    fetchPayments();
  }, [currentPage, searchTerm, statusFilter, screenSize]);
  ...
  // Enhanced icon sizing with better responsive scaling
  const getStatusIcon = (status, size = "default") => {
  ...
  const PaymentModal = () => {
    ...
    return (
      <div 
        className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 sm:p-6 backdrop-blur-sm"
        onClick={() => setShowPaymentModal(false)}
      >
        <div 
          className="bg-white rounded-xl sm:rounded-2xl w-full max-w-2xl lg:max-w-4xl max-h-[90vh] sm:max-h-[95vh] overflow-y-auto shadow-2xl transform transition-all duration-300 mx-auto"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white rounded-t-xl sm:rounded-t-2xl">
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm sm:text-base text-gray-600">Complete transaction information</p>
              </div>
            </div>
            <button
              onClick={() => setShowPaymentModal(false)}
          ...
          <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              <div className="md:col-span-2 lg:col-span-2 bg-gradient-to-br from-blue-50 to-white rounded-xl p-4 sm:p-6 border border-blue-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                  ...
                  <div className={`px-3 py-2 sm:px-4 sm:py-2 rounded-full text-sm font-bold border-2 ${getStatusColor(selectedPayment.payment?.status)} flex-shrink-0 mt-2 sm:mt-0`}>
                    <div className="flex items-center space-x-2">
                      {getStatusIcon(selectedPayment.payment?.status)}
                      <span className="truncate">{selectedPayment.payment?.status}</span>
                    </div>
              ...
              {/* Amount Card */}
              <div className="bg-gradient-to-br from-green-50 to-white rounded-xl p-4 sm:p-6 border border-green-100">
                <h4 className="font-semibold text-gray-900 mb-3 sm:mb-4 text-lg">Payment Amount</h4>
                <div className="text-center">
                  <p className="text-2xl sm:text-3xl lg:text-4xl font-bold text-green-600 break-words">
                    {formatCurrency(selectedPayment.payment?.amount || selectedPayment.price)}
                  </p>
                  <p className="text-sm sm:text-base text-gray-600 mt-2">Service Fee</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div className="bg-gradient-to-br from-purple-50 to-white rounded-xl p-4 sm:p-6 border border-purple-100">
                <h4 className="font-semibold text-gray-900 mb-3 sm:mb-4 text-lg flex items-center space-x-2">
              ...
              {/* Service Details */}
              <div className="bg-gradient-to-br from-orange-50 to-white rounded-xl p-4 sm:p-6 border border-orange-100">
                <h4 className="font-semibold text-gray-900 mb-3 sm:mb-4 text-lg flex items-center space-x-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-4 sm:p-6 border border-blue-100">
                ...
                <div className="flex items-center space-x-3 sm:space-x-4">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center shadow-lg flex-shrink-0">
              ...
              <div className="bg-gradient-to-br from-green-50 to-white rounded-xl p-4 sm:p-6 border border-green-100">
                ...
                <div className="flex items-center space-x-3 sm:space-x-4">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-green-500 to-blue-500 rounded-full flex items-center justify-center shadow-lg flex-shrink-0">
                    <User className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-gray-900 text-lg truncate">
          ...
          <div className="flex flex-col sm:flex-row gap-3 p-4 sm:p-6 border-t border-gray-200 bg-gray-50 rounded-b-xl sm:rounded-b-2xl">
            <button
              onClick={() => setShowPaymentModal(false)}
              className="flex-1 px-6 py-4 bg-white border-2 border-gray-300 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 shadow-md hover:shadow-lg text-base"
            >
              Close Details
            </button>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4 hover:shadow-lg transition-all duration-300 hover:border-blue-300">
        <div className="flex justify-between items-start mb-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <div className="min-w-0">
                <p className="font-mono text-sm text-gray-900 font-bold truncate">
                  #{payment.payment?.paymentId?.slice(-8) || payment._id.slice(-8)}
                </p>
                <p className="text-xs text-gray-600">
                  {payment.payment?.transactionDate ?
          ...
          <div className={`px-3 py-1.5 rounded-full text-xs font-bold border-2 ${statusColor} flex-shrink-0 ml-2`}>
            <div className="flex items-center space-x-1">
              {getStatusIcon(payment.payment?.status, "small")}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-blue-50 rounded-lg p-3">
              <p className="text-xs font-medium text-gray-600">Customer</p>
              <p className="font-semibold text-gray-900 text-sm truncate">{payment.customerId?.name || 'N/A'}</p>
            </div>
            <div className="bg-green-50 rounded-lg p-3">
              <p className="text-xs font-medium text-gray-600">Worker</p>
              <p className="font-semibold text-gray-900 text-sm truncate">{payment.workerId?.name || 'N/A'}</p>
            </div>
          </div>

          <div className="flex justify-between items-center bg-purple-50 rounded-lg p-3">
            <span className="text-sm font-medium text-gray-600">Method:</span>
            <span className="font-semibold text-gray-900 text-sm">{payment.payment?.paymentType || 'N/A'}</span>
          </div>
        </div>
        ...
        <div className="flex justify-end mt-4 pt-4 border-t border-gray-100">
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-md hover:shadow-lg text-sm"
            aria-label={`View details for payment ${payment.payment?.paymentId?.slice(-8) || payment._id.slice(-8)}`}
          >
            <Eye className="w-4 h-4" />
            <span>View Details</span>
          </button>
  ...
  const StatCard = ({ title, value, growth, icon: Icon, color }) => {
    ...
    return (
      <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300">
        <div className="flex items-center justify-between mb-4">
        ...
        <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-2 break-words">
          {title === 'Total Revenue' ? formatCurrency(value) : value.toLocaleString()}
        </h3>
  ...
  const Pagination = () => {
    ...
    const getVisiblePages = () => {
      let maxVisible = screenSize.isMobile ? 3 : 5;
    ...
    return (
      <div className="flex flex-col sm:flex-row items-center justify-between px-4 sm:px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100 border-t-2 border-gray-200 gap-4">
        <p className="text-sm font-semibold text-gray-700 text-center sm:text-left">
          Page {currentPage} of {totalPages} • {payments.length} payments
        </p>
        <div className="flex items-center space-x-2 sm:space-x-3">
          <button
            onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
            disabled={currentPage === 1}
            className="p-2 sm:p-3 rounded-xl border-2 border-gray-300 hover:bg-white hover:border-gray-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-md hover:shadow-lg flex-shrink-0"
            aria-label="Previous page"
          >
          ...
          <div className="flex items-center space-x-1 sm:space-x-2">
            {getVisiblePages().map((pageNum) => (
              <button
                key={pageNum}
                onClick={() => setCurrentPage(pageNum)}
                className={`px-3 py-1 sm:px-4 sm:py-2 rounded-lg font-semibold transition-all duration-200 text-sm ${
                  currentPage === pageNum
                    ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg'
                    : 'text-gray-700 hover:bg-white hover:shadow-md border border-gray-200'
                }`}
            ...
            {totalPages > getVisiblePages().length && currentPage < totalPages - 1 && (
              <span className="px-2 text-gray-500 font-semibold text-sm">...</span>
            )}
          </div>
  ...
  // Enhanced Desktop Table Row
  const DesktopTableRow = ({ payment }) => (
    <tr key={payment._id} className="hover:bg-blue-50 transition-colors duration-200 group">
      <td className="px-4 sm:px-6 py-4">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-500 rounded-lg flex items-center justify-center shadow-md flex-shrink-0">
            <CreditCard className="w-5 h-5 text-white" />
          </div>
          <div className="min-w-0">
      ...
      <td className="px-4 sm:px-6 py-4">
        <div className="flex items-center space-x-3">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-gray-900 text-base truncate">{payment.customerId?.name || 'N/A'}</p>
            <p className="text-sm text-gray-500 truncate">{payment.customerId?.phone || 'N/A'}</p>
          </div>
        </div>
      </td>
      <td className="px-4 sm:px-6 py-4">
        <div className="flex items-center space-x-3">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-gray-900 text-base truncate">{payment.workerId?.name || 'N/A'}</p>
            <p className="text-sm text-gray-500 truncate">{payment.workerId?.phone || 'N/A'}</p>
          </div>
        </div>
      </td>
      <td className="px-4 sm:px-6 py-4">
      ...
      <td className="px-4 sm:px-6 py-4">
        <span className="font-semibold text-gray-900 text-base truncate">{payment.payment?.paymentType || 'N/A'}</span>
      </td>
      <td className="px-4 sm:px-6 py-4">
        <div className={`px-4 py-2 rounded-full text-sm font-bold border-2 ${getStatusColor(payment.payment?.status)}`}>
          <div className="flex items-center space-x-2">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-10xl mx-auto space-y-6 sm:space-y-8">
        {/* Enhanced Header */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 flex-shrink-0">
            <button
              onClick={handleRefresh}
              disabled={refreshLoading}
              className="p-3 sm:p-4 bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-200 hover:bg-gray-50 transition-colors duration-200 disabled:opacity-50 flex-shrink-0"
              title="Refresh data"
              aria-label="Refresh payments"
            >
              <RefreshCw className={`w-5 h-5 sm:w-6 sm:h-6 text-gray-600 ${refreshLoading ? 'animate-spin' : ''}`} />
            </button>
          </div>
        </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 sm:py-4 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-500 text-base transition-all duration-200"
                    aria-label="Search payments"
                  />
                </div>
                <button
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <div className="flex items-center space-x-3 bg-gray-50 rounded-xl p-3 border border-gray-200">
                ...
                <select
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value)}
                  className="bg-transparent border-none focus:outline-none focus:ring-0 text-gray-700 font-medium text-base w-full"
                  aria-label="Filter by status"
                >
                  {statuses.map(status => (
                    <option key={status.value} value={status.value}>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center h-48 sm:h-64">
              <div className="text-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
                <p className="text-base text-gray-600 font-medium">Loading payments...</p>
              </div>
            </div>
          ) : (
            <>
              {screenSize.isDesktop ? (
                    ...
                    <table className="w-full">
                      <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b-2 border-gray-200">
                        <tr>
                          {[
                            ...
                            <th
                              key={header}
                              className="px-4 sm:px-6 py-4 text-left text-sm font-bold text-gray-900 uppercase tracking-wider"
                            >
                              {header}
                            </th>
                          ))}
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {payments.length === 0 ? (
                          ...
                          payments.map((payment) => (
                            <DesktopTableRow key={payment._id} payment={payment} />
                          ))
                        )}
                      </tbody>
                    </table>
                ...
                <div className="p-4 sm:p-6">
                  {payments.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-48 bg-gradient-to-br from-gray-50 to-white rounded-2xl border-2 border-dashed border-gray-300 p-6">
                      <CreditCard className="w-16 h-16 text-gray-400 mb-4" />
                      <p className="text-lg font-semibold text-gray-600 mb-2 text-center">No payments found</p>
                      <p className="text-gray-500 text-center max-w-sm">
                        Try adjusting your filters or search term to find payments
                      </p>
                    </div>
