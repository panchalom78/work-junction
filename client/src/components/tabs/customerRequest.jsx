...
const NonSmartphoneWorkers = () => {
  ...
  const ProgressDots = ({ b }) => {
    ...
    const done = b.serviceCompletedAt ? 4 :
                 b.serviceStartedAt ? 3 :
                 b.serviceInitiatedAt ? 2 : 1;
    ...
    return (
      <div className="flex items-center space-x-1">
        {[1, 2, 3, 4].map(i => (
          <div
            key={i}
            className={`w-1.5 h-1.5 rounded-full transition-all ${i <= done ? 'bg-green-500' : i === idx + 1 ? 'bg-blue-500 animate-pulse' : 'bg-gray-300'}`}
          />
        ))}
      </div>
    );
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 hover:shadow-md transition-all">
        ...
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center space-x-2 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-gray-600 truncate">{c.phone || '—'}</p>
            </div>
          </div>
          <div className="text-right flex-shrink-0 ml-2">
            ...
            <p className="text-[10px] text-gray-500 mt-0.5">
              {safeFormat(b.bookingInfo?.date)} • {b.bookingInfo?.time || '—'}
            </p>
          </div>
        </div>
        ...
        <div className="bg-blue-50 p-2 rounded-md mb-2 text-xs">
          <p className="truncate"><strong>Service:</strong> {s.name || '—'}</p>
          <p className="truncate"><strong>Skill:</strong> {sk}</p>
          <p className="font-bold text-green-600 text-sm">₹{b.serviceDetails?.price || 0}</p>
        </div>

        {/* Customer Address - Compact */}
        <div className="bg-gray-50 p-2 rounded-md mb-2 text-[11px]">
          <p className="font-medium text-gray-800 mb-0.5">Customer Address</p>
          <p className="truncate">{c.address?.area || '—'}, {c.address?.city || '—'}</p>
          <p className="truncate">{c.address?.state || '—'} - {c.address?.pincode || '—'}</p>
          <a href={`tel:${c.phone}`} className="text-blue-600 font-medium mt-0.5 inline-block text-xs">
            📞 Call Customer
          </a>
        </div>
        ...
        {w.name && (
          <div className="bg-teal-50 p-2 rounded-md mb-2 text-[11px]">
            <p className="font-medium text-teal-800 mb-0.5">Assigned Worker</p>
            <p className="truncate"><strong>Name:</strong> {w.name}</p>
            <p className="truncate"><strong>Phone:</strong> {w.phone}</p>
            <a href={`tel:${w.phone}`} className="text-teal-600 font-medium mt-0.5 inline-block text-xs">
              📞 Call Worker
            </a>
          </div>
        )}
        ...
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-[10px] text-gray-600">
            {b.serviceCompletedAt ? 'Done' :
             b.serviceStartedAt ? 'Started' :
             b.serviceInitiatedAt ? 'Reached' :
             w.name ? 'Assigned' : 'Pending'}
          </span>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <button
            onClick={() => {
              const msg = `Hi ${w.name}, new job:\nCustomer: ${c.name}\nService: ${s.name}\nAddress: ${c.address?.area}, ${c.address?.city}\nDate: ${safeFormat(b.bookingInfo?.date)} ${b.bookingInfo?.time}\nCall: ${c.phone}`;
              window.open(`sms:${w.phone}?body=${encodeURIComponent(msg)}`, '_blank');
            }}
            disabled={!w.phone}
            className="w-full py-2 bg-green-600 text-white text-xs rounded-md hover:bg-green-700 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
          >
            📱 Send SMS
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50/30 p-2 space-y-3">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex justify-between items-start">
            <div className="flex-1 min-w-0">
              <h1 className="text-lg font-bold text-gray-800 truncate">Non-Smartphone Worker Jobs</h1>
              <p className="text-gray-600 text-xs">Agent-created workers only</p>
            </div>
            <button
          ...
          {tabs.map(t => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md font-medium text-xs whitespace-nowrap transition-all flex-shrink-0 ${
                activeTab === t.id ? 'bg-blue-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
            >
              {t.label}
            </button>
      ...
      <div className="grid grid-cols-1 gap-2">
        {filteredRequests.map(b => <BookingCard key={b._id} b={b} />)}
      </div>
      ...
      {filteredRequests.length === 0 && (
        <div className="text-center py-8 text-gray-500 bg-white rounded-lg shadow-sm">
          ...
          <p className="text-sm font-medium text-gray-900">No {activeTab} jobs found</p>
          <p className="text-xs text-gray-600 mt-0.5">Try changing your search or filters</p>
        </div>
      )}
              ...
              <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-200">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-600 mt-0.5 truncate">
                    Booking ID: <span className="font-mono">#{selectedBooking._id?.slice(-6) || '—'}</span>
                  </p>
                </div>
                <button
