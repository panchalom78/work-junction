...
const NonSmartphoneWorkerDashboard = () => {
    ...
    const safeDateTimeFormat = (dateStr, fallback = "—") => {
        ...
        try {
            ...
            return isNaN(d.getTime())
                ? fallback
                : format(d, "dd MMM, yyyy 'at' hh:mm a");
        } catch {
            return fallback;
        }
    };
    ...
    const fetchAgentData = async () => {
        try {
            ...
            // Get agent profile
            const agentResponse = await axiosInstance.get(
                "/api/service-agent/profile"
            );
    ...
    const fetchWorkerDetails = async (workerId) => {
        try {
            ...
            const workerResponse = await axiosInstance.get(
                `/api/service-agent/worker-details/${workerId}`
            );
            if (workerResponse.data.success) {
                ...
                if (data.worker) {
                    ...
                    setWorkerServices(
                        data.worker.workerProfile?.services ||
                            data.services ||
                            []
                    );
    ...
    const fetchAgentBookings = async (workersData) => {
        try {
            ...
            for (const worker of workersData) {
                try {
                    const bookingsResponse = await axiosInstance.get(
                        `/api/service-agent/bookings/${worker._id}`
                    );
                    if (bookingsResponse.data.success) {
                        const workerBookings =
                            bookingsResponse.data.data?.bookings || [];
                        // Add worker info to each booking
                        const bookingsWithWorker = workerBookings.map(
                            (booking) => ({
                                ...booking,
                                worker: {
                                    _id: worker._id,
                    ...
                    console.error(
                        `Error fetching bookings for worker ${worker._id}:`,
                        error
                    );
                }
            }
            ...
            const sortedBookings = allBookingsData.sort(
            ...
            const workersWithStats = workersData.map((worker) => {
                const workerBookings = sortedBookings.filter(
                    (booking) => booking.worker?._id === worker._id
                );
                return {
                    ...worker,
                    stats: {
                        ...
                        pending: workerBookings.filter(
                            (b) => b.status === "PENDING"
                        ).length,
                        active: workerBookings.filter(
                            (b) =>
                                b.status === "ACCEPTED" ||
            ...
            setStats({
                ...
                completed: sortedBookings.filter(
                    (b) => b.status === "COMPLETED"
                ).length,
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-6">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex justify-between items-center py-2 border-b border-gray-200">
                                                    ...
                                                    <span
                                                        className={`px-3 py-1 rounded-full text-xs font-medium ${availabilityStatus.bg} ${availabilityStatus.color}`}
                                                    >
                                                        {
                                                ...
                                                <div className="flex justify-between items-center py-2">
                                                    ...
                                                    <span
                                                        className={`px-3 py-1 rounded-full text-xs font-medium ${verificationStatus.bg} ${verificationStatus.color}`}
                                                    >
                                                        {
                                                        ...
                                                        (service, index) => {
                                                            ...
                                                            return (
                                                                <div
                                                                    ...
                                                                    className="bg-white rounded-lg p-4 border border-gray-200 z-50"
                                                                >
                                            ...
                                            worker.workerProfile.skills.length >
                                                ...
                                                <div className="bg-gray-50 rounded-lg p-6">
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        {worker.workerProfile.skills.map(
                                                            (skill, index) => (
                                                                <span
                                                                    key={
                                                                        skill._id ||
                        ...
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2 text-gray-600">
                                ...
                                <span className="text-sm">
                                    Created{" "}
                                    {safeDateTimeFormat(worker.createdAt)}
                                </span>
                            </div>
        ...
        <div className="bg-white rounded-lg md:rounded-xl p-4 md:p-6 shadow-sm border border-gray-200 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                ...
                >
                    ...
                    <p className="font-medium text-green-700 text-sm md:text-base">
                        Add Worker
                    </p>
                ...
                >
                    ...
                    <p className="font-medium text-blue-700 text-sm md:text-base">
                        All Workers
                    </p>
                ...
                >
                    ...
                    <p className="font-medium text-orange-700 text-sm md:text-base">
                        Pending Jobs
                    </p>
    ...
    const WorkerCard = ({ worker, compact = false }) => {
        ...
        return (
            <div
                onClick={() => handleWorkerCardClick(worker)}
                className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm hover:shadow-md transition-all cursor-pointer z-50"
            >
                <div className="flex items-center gap-3">
    ...
    const BookingCard = ({ booking, compact = false }) => {
        ...
        const getStatusIcon = (status) => {
            switch (status) {
                ...
                case "PAYMENT_PENDING":
                    return (
                        <Wrench className="w-3 h-3 md:w-4 md:h-4 text-purple-600" />
                    );
                case "COMPLETED":
                    return (
                        <Trophy className="w-3 h-3 md:w-4 md:h-4 text-green-600" />
                    );
                default:
                    return (
                        <AlertCircle className="w-3 h-3 md:w-4 md:h-4 text-gray-600" />
                    );
            }
        };
                        ...
                        <div className="flex justify-between items-center">
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-gray-600 text-xs truncate">
                                    {service.skillName || "General Service"}
                                </p>
                            </div>
                    ...
                    <div className="flex gap-1 flex-wrap">
                        {actions.map((action, index) => (
                            <button
                                key={index}
                                onClick={() =>
                                    handleQuickAction(booking, action.action)
                                }
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <p className="text-gray-500 text-xs">
                                Assigned to: {booking.worker?.name || "—"}
                            </p>
                        </div>
                    </div>
                    <div className="text-right flex-shrink-0 ml-2">
                        ...
                        <p className="text-gray-500 text-xs flex items-center gap-1 justify-end">
                            ...
                            {safeFormat(booking.bookingInfo?.date)} •{" "}
                            {safeTimeFormat(booking.bookingInfo?.time)}
                        </p>
                    </div>
                </div>
                ...
                {bookings.length > 0 ? (
                    ...
                    <div className="text-center py-6 md:py-8 text-gray-500">
                        ...
                        <p className="text-xs md:text-sm mt-1">
                            Bookings from your workers will appear here
                        </p>
                    </div>
                )}
            </div>
            ...
            <div className="space-y-3">
                {workers.length > 0 ? (
                    ...
                    <div className="text-center py-6 md:py-8 text-gray-500">
                        ...
                        <p className="text-xs md:text-sm mt-1">
                            Add your first non-smartphone worker
                        </p>
                        <button
                            onClick={() => setActiveView("createWorker")}
        ...
        <div className="bg-white rounded-lg md:rounded-xl p-4 md:p-6 shadow-sm border border-gray-200">
            ...
            <div className="space-y-3 md:space-y-4">
                {recentActivity.map((activity, index) => (
                    <div
                        key={activity.id}
                        className="flex items-start gap-2 md:gap-3 p-2 md:p-3 hover:bg-gray-50 rounded-lg transition-colors"
                    >
                        <div
                        ...
                        >
                            ...
                            {activity.type === "work_completed" && (
                                <Trophy className="w-3 h-3 md:w-4 md:h-4 text-orange-600" />
                            )}
                        </div>
                        <div className="flex-1 min-w-0">
    ...
    // ALL WORKERS VIEW
    // ALL WORKERS VIEW - FIXED VERSION
    const AllWorkersView = () => (
        <div className="min-h-screen bg-gray-50">
                ...
                {workers.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-500 mb-6">
                            Add your first non-smartphone worker to get started
                        </p>
                        <button
                            onClick={() => setActiveView("createWorker")}
            ...
            {/* Render WorkerDetailsModal here as well for AllWorkersView */}
            ...
            <div className="bg-white rounded-lg md:rounded-xl p-4 md:p-6 shadow-sm border border-gray-200 animate-pulse">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                    {[1, 2, 3, 4].map((i) => (
                        <div
                            key={i}
                            className="p-3 md:p-4 bg-gray-100 rounded-lg"
                        >
