...
const WorkerVerificationPage = () => {
  ...
  const { status, loading, error, message, fetchStatus, uploadAllDocuments, deleteDocument } = useVerificationStore();
  ...
  const handleSubmitDocuments = async () => {
    const allDocumentsUploaded = uploadedDocuments.aadhar && uploadedDocuments.selfie;
    ...
    if (uploadedDocuments.policeVerification) {
      formData.append('policeVerification', uploadedDocuments.policeVerification);
    }
  ...
  const getVerificationHistory = () => {
    ...
    if (status.verificationStatus === 'REJECTED' && status.rejectedAt) {
      history.push({
        ...
        details: status.rejectionReason || 'Additional information required',
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-100 border border-green-200 rounded-2xl p-6 mb-8">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              <div className="bg-white rounded-lg p-3 shadow-sm">
                ...
                <p className="text-green-700">{formatDate(status.verifiedAt)}</p>
              </div>
              <div className="bg-white rounded-lg p-3 shadow-sm">
                <span className="font-semibold text-green-800">Approved By:</span>
                <p className="text-green-700">{status.verifiedBy?.name || 'Verification Agent'}</p>
              </div>
            </div>
          </div>
        )}
            ...
            {showRejectionDetails && (
              <div className="space-y-4 bg-white rounded-lg p-4 shadow-sm">
                <div>
                  ...
                  <p className="text-red-700 mt-2 p-3 bg-red-50 rounded-lg">{status.rejectionReason || 'Additional information required'}</p>
                </div>
                <div className="grid md:grid-cols-2 gap-4 text-sm">
                  <div className="bg-red-50 rounded-lg p-3">
                    <span className="font-semibold text-red-800">Rejected On:</span>
                    <p className="text-red-700">{formatDate(status.rejectedAt)}</p>
                  </div>
                  <div className="bg-red-50 rounded-lg p-3">
                    <span className="font-semibold text-red-800">Reviewed By:</span>
                    <p className="text-red-700">{status.rejectedBy?.name || 'Verification Agent'}</p>
                  </div>
                </div>
              </div>
            )}
              ...
              getVerificationHistory().map((event, index) => (
                <div key={index} className="flex space-x-4 relative">
                  <div className="flex flex-col items-center">
                    ...
                    {index < getVerificationHistory().length - 1 && (
