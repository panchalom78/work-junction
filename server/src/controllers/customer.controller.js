...
// -----------------------------
// GET ALL WORKERS (simple, paginated)
// -----------------------------
export const getWorkerList = asyncHandler(async (req, res) => {
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 10;

  const workerFilter = { role: "WORKER", isVerified: true };
  ...
  const workerServices = await WorkerService.find({ workerId: { $in: workerIds } })
    .populate("skillId", "name")
    .lean();

  const enrichedWorkers = workers.map((worker) => {
    const services = workerServices.filter(
...
// -----------------------------
// SEARCH SERVICES (with filters, pagination, rating, distance)
// -----------------------------
const searchServices = asyncHandler(async (req, res) => {
  ...
  let workerFilter = { role: "WORKER", isVerified: true };
  if (city) workerFilter["address.city"] = new RegExp(city, "i");
  if (pincode) workerFilter["address.pincode"] = pincode;
  if (!city && !pincode && customer?.address?.city)
    workerFilter["address.city"] = new RegExp(customer.address.city, "i");
  if (workerName) workerFilter.name = new RegExp(workerName, "i");
  if (workerPhone) workerFilter.phone = new RegExp(workerPhone);

  const workers = await User.find(workerFilter).select("_id name phone address");
  const workerIds = workers.map((w) => w._id);

  if (!workerIds.length) {
    return successResponse(res, 200, "No workers found", { workers: [], pagination: {} });
  }

  const serviceFilter = { workerId: { $in: workerIds } };
  if (skill) {
    const skillDoc = await Skill.findOne({ name: new RegExp(`^${skill}$`, "i") });
    if (!skillDoc)
      return successResponse(res, 200, "No results for skill", { workers: [], pagination: {} });
    serviceFilter.skillId = skillDoc._id;

    if (service) {
      const serviceDoc = skillDoc.services.find(
        (s) => s.name.toLowerCase() === service.toLowerCase()
      );
      if (!serviceDoc)
        return successResponse(res, 200, "No results for service", { workers: [], pagination: {} });
      serviceFilter.serviceId = serviceDoc.serviceId;
    }
  }

  if (minPrice || maxPrice) {
    serviceFilter.price = {};
    if (minPrice) serviceFilter.price.$gte = parseFloat(minPrice);
    if (maxPrice) serviceFilter.price.$lte = parseFloat(maxPrice);
  }

  const workerServices = await WorkerService.find(serviceFilter)
    .populate("workerId", "name phone address")
    .populate("skillId", "name")
    .lean();

  const workerRatings = await Booking.aggregate([
    { $match: { workerId: { $in: workerIds }, "review.rating": { $exists: true } } },
    { $group: { _id: "$workerId", avgRating: { $avg: "$review.rating" }, ratingCount: { $sum: 1 } } },
  ]);
  const ratingsMap = {};
  workerRatings.forEach((r) => (ratingsMap[r._id.toString()] = r));

  const customerLocation =
    customer?.address?.coordinates?.latitude && customer?.address?.coordinates?.longitude
      ? {
          lat: parseFloat(customer.address.coordinates.latitude),
          lng: parseFloat(customer.address.coordinates.longitude),
        }
      : null;

  let enrichedServices = workerServices.map((ws) => {
    const workerId = ws.workerId._id.toString();
    const rating = ratingsMap[workerId] || { avgRating: 0, ratingCount: 0 };
    let distance = null;
    if (customerLocation && ws.workerId.address?.coordinates?.latitude) {
      distance = calculateDistance(
        customerLocation.lat,
        customerLocation.lng,
        parseFloat(ws.workerId.address.coordinates.latitude),
        parseFloat(ws.workerId.address.coordinates.longitude)
      );
    }
    return {
      workerServiceId: ws._id,
      workerId: ws.workerId._id,
      workerName: ws.workerId.name,
      workerPhone: ws.workerId.phone,
      skill: ws.skillId.name,
      serviceId: ws.serviceId,
      details: ws.details,
      pricingType: ws.pricingType,
      price: ws.price,
      portfolioImages: ws.portfolioImages || [],
      avgRating: rating.avgRating,
      ratingCount: rating.ratingCount,
      distance,
      address: ws.workerId.address,
    };
  });

  if (minRating || maxRating) {
    enrichedServices = enrichedServices.filter((ws) => {
      if (minRating && ws.avgRating < parseFloat(minRating)) return false;
      if (maxRating && ws.avgRating > parseFloat(maxRating)) return false;
      return true;
    });
  }
  ...
  enrichedServices.sort((a, b) => {
    switch (sortBy) {
      ...
      case "relevance":
      default:
        const scoreA = a.avgRating * Math.log(a.ratingCount + 1);
        const scoreB = b.avgRating * Math.log(b.ratingCount + 1);
        return scoreB - scoreA;
    }
  });

  const startIndex = (page - 1) * limit;
  const paginatedServices = enrichedServices.slice(startIndex, startIndex + parseInt(limit));

  const pagination = {
    currentPage: parseInt(page),
    totalPages: Math.ceil(enrichedServices.length / limit),
    totalResults: enrichedServices.length,
    resultsPerPage: parseInt(limit),
  };

  return successResponse(res, 200, "Services retrieved successfully", {
    workers: paginatedServices,
    pagination,
  });
});
