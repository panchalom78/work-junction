...
export const getAgentStats = async (req, res) => {
    ...
    try {
        const agent = await ServiceAgent.findOne({ userId }).select(
            "completedVerifications"
        );
        if (!agent) {
            return res
                .status(404)
                .json({ success: false, message: "Service agent not found." });
        }
        ...
        const stats = {
            verificationsCompleted: {
                ...
                target: weeklyTarget,
                progress: Math.min(
                    (completedVerifications / weeklyTarget) * 100,
                    100
                ).toFixed(0),
            },
        };
...
export const setupServiceAgent = async (req, res) => {
    ...
    if (!address || !city || !state || !pincode || !location?.coordinates) {
        return res.status(400).json({
            success: false,
            message:
                "All required fields (address, city, state, pincode, location.coordinates) must be provided.",
        });
    }
    ...
    try {
        ...
        if (user.role !== "SERVICE_AGENT") {
            ...
            return res.status(403).json({
                success: false,
                message: "User must have SERVICE_AGENT role.",
            });
        }
...
export const getAreaStats = async (req, res) => {
    ...
    try {
        ...
        if (!user || user.role !== "SERVICE_AGENT") {
            return res.status(403).json({
                success: false,
                message: "User must be a SERVICE_AGENT.",
            });
        }
        ...
        const assignedThisWeek = await User.countDocuments({
            ...
            "workerProfile.verification.serviceAgentId": userId,
            "workerProfile.verification.verifiedAt": {
                $gte: startOfWeek,
                $lte: endOfWeek,
            },
        });
        ...
        const previousActiveWorkers = agent.activeWorkers
            ? Math.max(agent.activeWorkers - 18, 0)
            : 0;
        const previousPendingRequests = pendingRequests
            ? Math.max(pendingRequests - 1, 0)
            : 0;
        const previousAssignedThisWeek = assignedThisWeek
            ? Math.max(assignedThisWeek - 5, 0)
            : 0;
...
export const getWorkersForVerification = async (req, res) => {
    try {
        ...
        const workers = await User.find({
            ...
            .select("name email phone address workerProfile.verification")
        ...
        const processedServices = await Promise.all(
            workerServices.map(async (service) => {
                ...
                return {
                    ...
                    durationUnit:
                        service.pricingType === "HOURLY" ? "hours" : "fixed",
        ...
        if (req.user.role === "SERVICE_AGENT") {
            ...
            if (!serviceAgent) {
                return errorResponse(
                    res,
                    404,
                    "Service agent profile not found"
                );
            }

            const agentArea =
                Array.isArray(serviceAgent.areasAssigned) &&
                serviceAgent.areasAssigned.length > 0
        ...
        if (req.user.role === "SERVICE_AGENT") {
            ...
            if (agentArea && worker.address?.area !== agentArea) {
                return errorResponse(
                    res,
                    403,
                    "Access denied for this worker’s area"
                );
            }
        }
...
// ==================== GET ALL WORKERS ====================
// In your backend controller for all-workers
// ==================== GET SKILLS ====================
export const getSkills = async (req, res) => {
    try {
        ...
        // Transform services to include serviceId as string for frontend
        const transformedSkills = skills.map((skill) => ({
            ...skill,
            services: skill.services.map((service) => ({
                ...service,
...
// Add this to your worker controller
// ==================== GET WORKER COUNTS ====================
// ==================== GET WORKER COUNTS ====================
export const getWorkerCounts = async (req, res) => {
    try {
        // Get all active workers
        const workers = await User.find({ role: "WORKER" }).lean();

        // Get all worker services with populated skill and service info
        const workerServices = await WorkerService.find({ isActive: true })
            .populate({
                path: "skillId",
                select: "name",
            })
        ...
        // Count by skill - from WorkerService model (more accurate)
        workerServices.forEach((service) => {
            if (service.skillId && service.skillId._id) {
                const skillId = service.skillId._id.toString();
                counts.bySkill[skillId] = (counts.bySkill[skillId] || 0) + 1;
            }
        });

        // Count by service - from WorkerService model
        workerServices.forEach((service) => {
            if (service.serviceId) {
                const serviceId = service.serviceId.toString();
                counts.byService[serviceId] =
                    (counts.byService[serviceId] || 0) + 1;
            }
        });
