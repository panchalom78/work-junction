...
export const getAgentDashboardStats = async (req, res) => {
    try {
        ...
        if (!serviceAgent) {
            return res
                .status(404)
                .json({ error: "Service agent profile not found" });
        }

        // Get workers created by this agent
        const workers = await User.find({
            "workerProfile.verification.serviceAgentId": agentId,
        });

        const totalWorkers = workers.length;
        const activeWorkers = workers.filter(
            (worker) => worker.workerProfile?.availabilityStatus === "available"
        ).length;
        ...
        const completedBookingsWithReview = bookings.filter(
            (booking) => booking.status === "COMPLETED" && booking.review
        );

        const customerRating =
            completedBookingsWithReview.length > 0
                ? completedBookingsWithReview.reduce(
                      (total, booking) => total + booking.review.rating,
                      0
                  ) / completedBookingsWithReview.length
                : 0;
    ...
    try {
        ...
        const activities = recentBookings.map((booking) => {
            ...
            switch (booking.status) {
                case "PENDING":
                    type = "new_request";
                    message = `New ${getServiceType(booking)} request from ${
                ...
                case "ACCEPTED":
                    type = "assignment";
                    message = `Assigned ${getServiceType(booking)} to ${
                        booking.customerId?.name
                    }`;
                    status = "completed";
                    break;
                case "COMPLETED":
                    type = "completion";
                    message = `${getServiceType(booking)} completed for ${
                        booking.customerId?.name
                    }`;
                    status = "completed";
                    break;
                case "CANCELLED":
                    type = "cancellation";
                    message = `${getServiceType(booking)} cancelled by ${
                        booking.customerId?.name
                    }`;
                    status = "completed";
                    break;
    ...
    try {
        ...
        workers.forEach((worker) => {
            worker.workerProfile?.skills?.forEach((skill) => {
                ...
                skillDistribution[skillName] =
                    (skillDistribution[skillName] || 0) + 1;
            });
        });

        // Calculate smartphone vs non-smartphone distribution
        const smartphoneWorkers = workers.filter(
            (w) => w.workerProfile.createdByAgent === true
        ).length;
        ...
        const distribution = {
            smartphoneUsers: {
                percentage:
                    workers.length > 0
                        ? (smartphoneWorkers / workers.length) * 100
                        : 0,
            ...
            nonSmartphoneUsers: {
                percentage:
                    workers.length > 0
                        ? (nonSmartphoneWorkers / workers.length) * 100
                        : 0,
        ...
        if (period === "month") {
            ...
            earningsData = weeks.map((week, index) => {
                ...
                const amount = weekBookings.reduce((total, booking) => {
                    return (
                        total + (booking.payment?.amount || booking.price || 0)
                    );
                }, 0);
